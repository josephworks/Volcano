...
echo "Importing Volcano mc-dev"
./scripts/importmcdev.sh "$basedir" || exit 1
basedir=$(dirname ${basedir})

# Apply Volcano
(
    applyPatch Paper/Paper-API Volcano-API HEAD &&
    applyPatch Paper/Paper-Server Volcano-Server HEAD
) || (
    echo "Failed to apply Volcano Patches"
    exit 1
) || exit 1
) || exit 1
